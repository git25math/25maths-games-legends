    ...
    discoverSteps: [
      {
        ...
        acceptPattern: '',
        onCorrect: { zh: '对！3 人一排，排 4 排，正好 12 人，没有人多出来。\n\n像 3 这样能把 12 刚好分完的数，有个名字叫——"因数"。\n你刚才做的事情，就是在找因数！', en: 'Yes! 3 per row, 4 rows, exactly 12 — nobody left over.\n\nA number like 3 that divides 12 perfectly has a name — "factor".\nWhat you just did? That\'s finding factors!' },
        onWrong: { zh: '数数看：3, 6, 9, 12。数到 12 了！\n说明 12 ÷ 3 = 4，刚好分完，没人多出来。\n\n能把 12 刚好分完的数叫"因数"。3 就是 12 的一个因数。', en: 'Count: 3, 6, 9, 12. You reached 12!\nSo 12 ÷ 3 = 4, divides perfectly, nobody left over.\n\nA number that divides 12 perfectly is called a "factor". 3 is a factor of 12.' },
        onSkip: { zh: '我来带你数：3, 6, 9, 12——数到了！\n3 个人一排，4 排，正好 12 人。\n\n能把 12 刚好分完的数叫"因数"。\n比如 3 就是 12 的因数，因为 12 ÷ 3 没有余数。', en: 'Let me count with you: 3, 6, 9, 12 — got there!\n3 per row, 4 rows, exactly 12.\n\nA number that divides 12 with nothing left over is called a "factor".\n3 is a factor of 12 because 12 ÷ 3 has no remainder.' },
      },
      {
        ...
        acceptPattern: '',
        onCorrect: { zh: '没错！12 ÷ 5 = 2 余 2，除不尽，所以 5 不是因数。\n\n能整除的：1, 2, 3, 4, 6, 12——一共 6 种排法。\n发现了吗？它们成对出现：1×12, 2×6, 3×4。\n接下来我们学习系统地找因数的方法！', en: 'Right! 12 ÷ 5 = 2 remainder 2 — doesn\'t divide evenly, so 5 is NOT a factor.\n\nFactors of 12: 1, 2, 3, 4, 6, 12 — that\'s 6 arrangements.\nNotice? They come in pairs: 1×12, 2×6, 3×4.\nNow let\'s learn the systematic method!' },
        onWrong: { zh: '算一下：12 ÷ 5 = 2，余 2。有余数！5 人一排会多出 2 人。\n所以 5 不是 12 的因数。\n\n最终能排整齐的：1, 2, 3, 4, 6, 12——一共 6 种。\n因数总是成对的：1×12, 2×6, 3×4。', en: 'Calculate: 12 ÷ 5 = 2 remainder 2. There\'s a leftover! 5 per row leaves 2 out.\nSo 5 is NOT a factor of 12.\n\nValid arrangements: 1, 2, 3, 4, 6, 12 — six total.\nFactors always pair up: 1×12, 2×6, 3×4.' },
        onSkip: { zh: '12 ÷ 5 = 2 余 2，除不尽。5 人一排不行！\n\n所有能排整齐的：1, 2, 3, 4, 6, 12。\n这 6 个数就是 12 的全部因数。\n\n小窍门：因数成对出现（1×12, 2×6, 3×4），所以只用试到一半！', en: '12 ÷ 5 = 2 remainder 2 — doesn\'t work. 5 per row fails!\n\nAll valid: 1, 2, 3, 4, 6, 12.\nThese 6 numbers are ALL factors of 12.\n\nTip: factors come in pairs (1×12, 2×6, 3×4), so check only up to halfway!' },
      },
    ],
    ...
    discoverSteps: [
      {
        ...
        acceptPattern: '',
        onCorrect: { zh: '对！4 份里吃了 1 份 = 四分之一。\n\n写成数学符号就是 $\\frac{1}{4}$——上面是你吃了几份（1），下面是一共切了几份（4）。\n分数就这么简单：几份里取几份。', en: 'Right! 1 out of 4 slices = one quarter.\n\nIn math notation: $\\frac{1}{4}$ — top is how many you ate (1), bottom is total slices (4).\nFractions are that simple: how many out of how many.' },
        onWrong: { zh: '想象一下：一块完整的披萨切成了 4 块，你只吃了 1 块。\n不是一半——一半是 2 块。你只吃了 1 块。\n\n1 块占 4 块的四分之一，写成 $\\frac{1}{4}$。', en: 'Picture it: a whole pizza cut into 4 slices, you ate just 1.\nNot half — half would be 2 slices. You only ate 1.\n\n1 out of 4 = one quarter, written as $\\frac{1}{4}$.' },
        onSkip: { zh: '4 块里吃了 1 块——这就是"四分之一"。\n\n写成分数：$\\frac{1}{4}$\n上面的 1 = 你吃了几块\n下面的 4 = 一共几块\n\n分数就是"几块里面取了几块"。', en: '1 slice out of 4 — that\'s "one quarter".\n\nAs a fraction: $\\frac{1}{4}$\nTop 1 = slices you ate\nBottom 4 = total slices\n\nA fraction just means "how many out of how many".' },
      },
      {
        ...
        acceptPattern: '',
        onCorrect: { zh: '没错！1 + 1 = 2 块。\n\n用分数写就是：$\\frac{1}{4} + \\frac{1}{4} = \\frac{2}{4}$\n\n发现规律了吗？分母（下面的 4）不变，分子（上面的数）相加！\n因为披萨还是切了 4 份，只是你们多拿了一份而已。', en: 'Yes! 1 + 1 = 2 slices.\n\nAs fractions: $\\frac{1}{4} + \\frac{1}{4} = \\frac{2}{4}$\n\nSee the pattern? The bottom number (4) stays the same, the top numbers add up!\nBecause the pizza is still 4 slices — you just took more of them.' },
        onWrong: { zh: '你吃了 1 块，朋友吃了 1 块。1 + 1 = 2 块！\n\n$\\frac{1}{4} + \\frac{1}{4} = \\frac{2}{4}$\n\n关键：下面的 4 不变（还是那个 4 块的披萨），上面的 1 + 1 = 2。', en: 'You had 1, friend had 1. 1 + 1 = 2 slices!\n\n$\\frac{1}{4} + \\frac{1}{4} = \\frac{2}{4}$\n\nKey: the bottom 4 stays (same pizza), top 1 + 1 = 2.' },
        onSkip: { zh: '1 块 + 1 块 = 2 块。\n\n分数也一样：$\\frac{1}{4} + \\frac{1}{4} = \\frac{2}{4}$\n\n秘诀：当下面的数（分母）一样时，只用把上面的数（分子）加起来！\n因为"切法"没变，只是"拿的份数"变多了。', en: '1 slice + 1 slice = 2 slices.\n\nSame with fractions: $\\frac{1}{4} + \\frac{1}{4} = \\frac{2}{4}$\n\nSecret: when the bottom numbers match, just add the top numbers!\nThe "slicing" didn\'t change — only "how many you took" changed.' },
      },
    ],
