...
| 2026-03-28 | **v9.8.0** | **三币经济闭环 + SIMILAR_TRIANGLES 新题型**: 军粮消费道具 `ration_pack`（补给包📦 修复力20，25军粮）+ `supply` 新类型 + `SHOP_PRICES` 接入 rations；`SIMILAR_TRIANGLES` 新 QuestionType + 生成器 + checker + 2 个 Y9 关卡（id 933/934，Unit 3 order 3-4，诸葛亮/赵云 6步教程）| **214关, 2289 tests ✓** |
