...
### Phase 2: 游戏化上瘾循环 (基于中学生游戏调研)

> **核心原则**：学习藏在游戏里，游戏驱动学习。让学生爱上"自己能解出难题"的感觉，而非依赖游戏刺激。

**调研结论（2025 年 12-18 岁学生最爱游戏机制）：**

| 游戏 | 机制 | 嫁接方式 |
|------|------|---------|
| 杀戮尖塔/Roguelike | 随机技能卡 | 每关 3 选 1 技能卡（双倍分/护盾/透视） |
| 原神 | 元素反应 | 多字段答对触发"融化反应"→ 双倍伤害 |
| 王者荣耀 | 段位系统 | 青铜→白银→黄金→钻石→王者 |
| Minecraft | 进度解锁 | 答对解锁地图格子，全通关触发彩蛋 |

| 任务 | 详情 | 优先级 |
|------|------|--------|
| 2.1 Roguelike 技能卡系统 | 每关开始前从卡池抽 3 选 1：双倍分/护盾(答错不扣血)/透视(显示正负号)/时间冻结 | P0 |
| 2.2 可变奖励 + 连击系统 | 答对随机触发：60%普通/25%稀有/10%技能升级/5%传说彩蛋 + 连击计数器 | P0 |
| 2.3 伤害数字飞出 + 连击特效 | 答对瞬间金色 "+250" 飞出 + "3连击！" 弹出 + 怪物血条下降 | P0 |
| 2.4 段位系统 | 青铜→白银→黄金→钻石→王者，绑定总功勋值 | P1 |
| 2.5 动态难度引擎 | 连对 3 题自动升难度，连错 2 题降难度 + "需要提示吗？" | P1 |
| 2.6 班级实时排行榜 | Supabase Realtime 订阅，30 秒刷新，超越通知 | P1 |

### Phase 3: 内容扩展

| 任务 | 详情 |
|------|------|
| 3.1 剩余 25 个关卡生成器 | LINEAR/QUADRATIC/TRIG/SIMULTANEOUS 等复杂类型 |
| 3.2 generate-all-missions.ts | 读 kp-registry + three-kingdoms → 生成 294 missions |
| 3.3 精品打磨 Topic 2.5 | 13 个方程 KP 完整故事 |
| 3.4 章节导航 + DAG 解锁 | 按史实编年 9 章地图 |

### Phase 4-6: 视频集成 → 多人 PK → 多主题
...
| 2026-03-17 | v1.3.0 | Phase C: 7 bug 修复 + i18n 完善 + 开发计划重构（融合游戏化调研） |
| — | v2.0.0 | Phase 2: Roguelike 技能卡 + 可变奖励 + 连击系统 + 段位 |
| — | v3.0.0 | Phase 3: 294 KP 全量关卡 + 章节导航 + DAG |
| — | v4.0.0 | Phase 4: 视频 + 练习三位一体 |
| — | v5.0.0 | Phase 5: 班级排行榜 + 多人 PK + 教师控制台 |
