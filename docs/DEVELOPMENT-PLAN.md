...
> **审查结论**：之前的计划把花哨游戏机制（技能卡/段位）放在内容和教育核心之前，导致"有机制无内容"的空壳风险。修正后：**教育价值 > 游戏机制 > 社交功能**。

**关键设计原则（基于严苛审查 15 项发现）：**

1. **生成器只改数字不改故事** — "桃园结义"永远是桃园结义，只是 `x+5=12` 变成 `x+7=15`
2. **答错比答对更重要** — 错误分析是教育价值最高的环节（参考 ExamHub Error Memory 5 类 pattern）
3. **多题连闯才有游戏深度** — 单题关卡无法支撑技能卡/连击等 Roguelike 机制
4. **40 个精品关卡 > 294 个平庸关卡** — Pareto 法则：40 个高频 KP 占 CIE 80% 分值
5. **Mobile-first** — 90% 用户在手机上

| # | 任务 | 详情 | 验收标准 | 工程量 |
|---|------|------|---------|--------|
| 2.1 | **生成器叙事修复** | 重构为"参数插值"模式：mission 存储带占位符的模板字符串（`已知 $\{a\}x=\{result\}$`），生成器只替换 `data.params`（a, x, result），渲染时插值。保留原始 title/story/角色。tutorialSteps 同理参数化。 | 同一 mission 多次进入，标题和角色不变，只有数字不同 | 大 — 重写 11 个生成器核心逻辑 |
| 2.2 | **答错解题展示** | 答错后展开解题面板（复用 AnimatedTutorial 组件），显示：① 你的答案 vs 正确答案 ② 逐步解题过程 ③ "再试一次"按钮。不需要完整 tutorialSteps，至少显示公式+正确值。 | 答错后能看到"你答了 8，正确答案是 7，因为 12-5=7" | 中 |
| 2.3 | **多题连闯模式** | MathBattle 内部管理题目队列（5 题，同 type 不同数字）。每题答对后 300ms 转场到下题。HP(4) 跨题共享。结算改为 N 题统计（正确数/总时间/连击峰值）。recordBattleComplete 改为批量记录。**注意：这是重写 MathBattle 约 60% 逻辑。** | 一关内连续做 5 题，中间不退出到地图 | 大 |
| 2.4 | **连击加成** | 连对 3 题 → ×1.5, 5 题 → ×2；答错清零。连击计数器 UI 显示在 header 旁。 | 纯前端实现，无需后端 | 小 |
| 2.5 | **伤害数字飞出** | 答对瞬间金色 "+250" 浮动上飘并淡出（absolute positioned, 1s animation）；连击时额外显示 "3连击！" | 300ms 内完成全部视觉反馈 | 小 |
...
| # | 任务 | 详情 | 验收标准 |
|---|------|------|---------|
| 3.1 | **40 个高频 KP 精品关卡** | 覆盖 CIE 0580 高频考点（方程/角度/面积/概率/三角/坐标/指数/统计），每个有完整故事+tutorial | 40 关全部有 Green 模式 tutorial + 生成器 |
| 3.2 | **专属复杂生成器** | LINEAR(双字段)/QUADRATIC/TRIG/SIMULTANEOUS/PROBABILITY_COMPLEMENT 等 | 华容道保留 `1-(2/3)^n` 结构，不降级 |
| 3.3 | **Roguelike 技能卡** | 多题连闯开始前 3 选 1：护盾(本轮 2 次免扣血)/双倍(后半段翻倍)/冻结(暂停计时 30s) | 每张卡在 5-8 题的连闯中有真实策略价值 |
| 3.4 | **动态难度** | 作用于生成器数字范围（Green 内部从 1-10 渐进到 1-50），不切换 G/A/R 模式 | 连对 3 题数字范围扩大，连错 2 题缩小 + "需要提示吗？" |
| 3.5 | **剧情分支（简版）** | 3 个关键关卡答错后显示"支线结局文字"（不实际跳转到另一个 mission）。完整 DAG 分支留到 Phase 5.1 与章节导航一起做。 | 答错勾股定理后出现"云梯太短！改走地道…"的叙事过渡 |
