...
> **版本**：v10.11.0 | 最后更新：2026-04-07
> **开发规范**：`docs/CONTRIBUTING.md`（适用于任何 AI/人类开发者）
> **Bug 防范**：`docs/BUG-POSTMORTEM.md`（15 条规则 + 根因分析）
> **优化全记录**：`docs/OPTIMIZATION-HISTORY.md`（v0.1→v7.5 全量分析）
...
- ✅ 模块化代码架构（`src/` 159 个文件，生成器已拆分到 `src/utils/generators/`）
- ✅ Green/Amber/Red 三级难度系统
- ✅ 424 个关卡（Y7 89 + Y8 78 + Y9 95 + Y10 87 + Y11 60 + Y12 15）
- ✅ 46 个题型 + 81 个活跃生成器类型 + 2,414 个 Vitest 用例
...
| 2026-03-24 | **v7.3.0** | **远征模式**: 赤壁远征 8 节点(green→red→boss) + 军粮系统(5起始/答错-1) + Boss 5x + 撤退安全阀 + 三种结局 |
| 2026-03-25 | **v7.4.0** | **成就墙 + 好友 PK**: 26 成就(征途/连签/赛季/收集) + PKSetupPanel(创建/加入) + LobbyScreen(房间码复制) + PKResultPanel(胜负动画) + useEscapeKey 全面板支持 |
| 2026-03-26 | **v7.5.0** | **多人 PK + 颁奖典礼 + 自动化测试**: 2-6 人 PK + 30s 倒计时 + 🥇🥈🥉 颁奖动画(confetti) + XP 排名加成(×2.0/1.5/1.2) + Vitest 1751 测试(77 生成器 ×20) | **5 轮审查 28 修复** |
| 2026-03-26 | **v8.0.0** | **教师看板 Phase A**: AlertPanel 预警面板(3d/7d未登录+进度落后+错题标红) + StudentDetailCard 7 维雷达图 + RadarChart SVG + DashboardScreen 模块化重构 |
| 2026-03-26 | **v8.1.0** | **Phase A.5+B**: 3 个 SECURITY DEFINER RPC(get_student_battles/get_class_kp_progress/get_class_battle_stats) + ClassOverview 4 卡片 + KPHeatmap 知识点×学生矩阵 |
| 2026-03-26 | **v8.2.0** | **Phase B.5+C**: WeeklyTrend 周报对比 + 多教师(TEACHER 标签) + CSV 导出 + 列排序(姓名/分数/进度/KP) | **教师看板 Phase A-C 全部完成** |
| 2026-03-26 | **v8.3.0** | **Phase D**: gl_assignments 表+4 RPC + AssignmentPanel(创建/归档/完成率追踪) + 学生端 AssignmentBanner(截止日期+进度环+直达按钮) + 关卡"老师布置"徽章 + ExamHubBridge 跨平台词汇掌握率 | **教师看板 Phase A-D 全部完成** |
| 2026-03-26 | **v8.4.0** | MathBattle 997→523 行拆分: BattleHeader(168)+BattleContent(164)+BattleEffects(67)+ResultOverlay(211) |
| 2026-03-26 | **v8.5.0** | Y9 Unit1-4 教程升级(10关3→6步) + Y9 新增 8 关(联立/二次/韦恩/扇形/旋转/放大/向量/累频) + **4轮审查26修复** + **3批UX优化8项** | **210 关卡, Y7-Y9 全金标准** |
| 2026-03-26 | **v8.6.0** | **全项目严格质量审计**: FUNC_VAL/ROOTS/PROBABILITY/TRIGONOMETRY 除零守卫 + FRAC容差0.001→0.01 + RATIO_RANDOM 缺 answer 字段修复 + mission 9143 type ANGLES→COORDINATES | **BUG归零** |
| 2026-03-26 | **v8.7.0** | **Y9/Y10/Y11 WHY升级**: Y9 10关+Y10 15关+Y11 5关教程第1步全部改为「为什么」叙事开场 | **Y9-Y11 叙事达标** |
| 2026-03-26 | **v8.8.0** | **Y8 全部40关6步金标准教程**: 从 `tutorialSteps:[]` 批量写入完整6步WHY→脚手架→微操作→答案→验算教程 | **Y8 教学全覆盖** |
| 2026-03-26 | **v8.9.0** | **Y12 金标准收尾**: `DERIVATIVE_RANDOM` 的 Y12 分支升级为 WHY→脚手架→微操作→答案→验算 7 步教程；mission `1211` 模板同步补齐 | **Y7-Y12 全部关卡达金标准** |
| 2026-03-26 | **v8.9.1** | **CI 质量门禁**: GitHub Actions 部署流程改为 `npm ci -> npm run lint -> npm test -- --run -> npm run build`，未通过即不部署 | **部署前强制质量校验** |
| 2026-03-26 | **v8.9.2** | **状态一致性 + 题目去重护栏**: 移除 daily challenge 对 `profile.completed_missions` 的原地 mutation；practice/battle 去重切换为稳定 question fingerprint；generator coverage 改为精确对齐注册表；新增 completion/question fingerprint 测试 | **状态模型更稳，Vitest 增至 1,780** |
| 2026-03-26 | **v8.9.3** | **generator 纯化 + 首屏拆包**: 去掉 generator 模块级 `tier` 全局状态并补 `generatorTiering` 回归测试；`missions` 改为动态加载；`Map/Lobby/Battle/Practice/Dashboard/PK/Expedition` 切到 `Suspense` 懒加载，主入口 `563.39 kB → 323.83 kB` | **首屏更轻，generator API 更稳** |
| 2026-03-27 | **v8.10.1** | **Bug 修复 + 性能**: ROOTS/QUADRATIC 除零守卫 + BugReportButton 左下角 + Calculator z-[55] + missions 按年级拆 6 chunk（557kB→~120kB，78% 减量）+ CI TS lint 修复 |
| 2026-03-27 | **v8.10.0** | **Bug 报告系统**: `gl_bug_reports` Supabase 表 + `BugReportButton` 组件（右下角浮动 + Modal + 4 类别 + 三语）集成到 PracticeScreen + MathBattle | **学生可一键反馈题目问题** |
| 2026-03-27 | **v9.0.0** | **背包系统 + 道具修复 + 科技树完整上线**: InventoryPanel 背包面板（MapScreen 🎒 入口）+ RepairDialog 道具修复弹窗（EquipmentPanel 内"🔨 使用道具"按钮）+ TechTreeScreen 双按钮任务卡（Practice/Battle）+ gameBalance.ts 游戏平衡常量统一管理 | **v9.0 阶段 1 完整上线** |
| 2026-03-27 | **v9.1.0** | **知识网络 + 错误补救闭环**: 94 条跨章节依赖完整图（CROSS_CHAPTER_PREREQS 5→94 边）+ errorRemediation.ts 55 话题×5 错误类型→前置补救映射(874 行) + Recovery Loop 递归修复路径（最深 ch1 基础→逐步回升→原节点恢复）+ RecoveryPathPanel UI + 安全加固（regex 注入/session 验证/14 天过期/stale closure 修复）| **2164 tests, 7 commits** |
| 2026-03-27 | **v9.1.1** | **状态层安全修复（3项 CRITICAL）**: 9处 `structuredClone` 深拷贝替换浅拷贝（规则 J）+ 7处 `latestScoreRef.current` 替换 `profile.total_score`（规则 K）+ techTree.ts TDZ 崩溃修复（`engineHealth` 先用后声明 → 前移声明，规则 L）+ 全项目 `var` 清零（→`const`/`let`）| **Build 零错误，2164 tests 全通过** |
| 2026-03-27 | **v9.2.0** | **5方向全量优化**: C1/C2 Resilience Engine入口（MapScreen腐败告警横幅+MathBattle失败"🔍诊断"按钮）+ DailyQuestPanel每任务独立按钮（战斗/练习分流）+ CIRCLE_THEOREM_RANDOM 圆周角定理生成器（Y10 Unit14 两关）+ 三币经济基础（功勋/智略/军粮 currency.ts + 战斗/练习自动发放 + MapScreen余额展示）| **212关, 2214 tests, 5 commits** |
| 2026-03-27 | **v9.5.0** | **Resilience Engine 全接入 + 精品课闭环 + UI 革命**: processAttempt()答题→技能健康实时更新 + detectErrorPattern()15规则识别 + 4张新Supabase表(user_skill_health/user_attempt_log/error_pattern_remedy_map/recovery_packs) + gl_lesson_runs + RepairScreen修复屏(诊断→5题→庆祝) + EXPAND_NEG_RANDOM生成器 + ExamHub精品课闭环(useLessonRecovery"特训完成！再试一次？") + 班级竞技场Top5 + 底部4-tab全局导航 + 67个sm响应式断点 + 页面过渡垂直淡入 + 微动效(火焰/辉光/Confetti) + 教师看板KP薄弱点排名 + **CI lint修复(4个TS类型错误)** | **212关, 2214 tests, CI绿灯** |
| 2026-03-27 | **v9.3.0** | **兵法宝典 + Error Pattern 扩展**: StrategicScrollsPanel 7篇锦囊（MapScreen桌面+移动端入口，三国人物封缄，三语支持）+ geometry新增5 pattern（parallel_line_angle/pythagoras_hypotenuse/area_formula_wrong/arc_sector_confusion/circle_theorem_error）+ trigonometry新增3 pattern（trig_inverse_confusion/sine_cosine_rule_choice/bearing_direction_error）+ detectErrorPattern 子话题精确路由 + errorRemediation ch4/4.7-4.9 具体补救路径 | **212关, 2214 tests** |
| 2026-03-27 | **v9.6.0** | **三币商店**: `ShopPanel.tsx`(余额展示+购买动画+不足提示+三语) + `buyItem()` + `SHOP_PRICES` 常量（🔨30功勋·📜50智略×4·💎80功勋）+ MapScreen 桌面/More菜单 🏪商店按钮 + App.tsx `onBuyItem` 回调（structuredClone 安全） | **212关, 2214 tests ✓** |
| 2026-03-27 | **v9.7.0** | **Y7 全量教程升级**: 修复 Y10 missions 10141/10142 空教程（圆周角+圆心角定理，诸葛亮/周瑜 6步）+ Y7 **45关**教程全升级（原1-5步 → 全部≥6步金标准）：BODMAS/分数/整数/代入/合并同类项/百分比/比例/质因数/HCF/LCM/数列/估算/几何/坐标/统计 15大类，三国叙事贯穿 | **212关, 2264 tests ✓, 4 commits** |
| 2026-03-28 | **v9.8.0** | **三币经济闭环 + SIMILAR_TRIANGLES 新题型**: 军粮消费道具 `ration_pack`（补给包📦 修复力20，25军粮）+ `supply` 新类型 + `SHOP_PRICES` 接入 rations；`SIMILAR_TRIANGLES` 新 QuestionType + 生成器 + checker + Y9 id=933（影子量旗）+ Y10 id=1065（田界测量）+ SVG两三角形并列图 + 16处story插值修复（Y7/Y9/Y10/Y11）| **216关, 2289 tests ✓** |
| 2026-03-28 | **v9.9.0** | **TREE_DIAGRAM 新题型（条件概率不放回）**: QuestionType + translations(3语) + inputConfig + checker(both_red/both_same/diff) + generateTreeDiagramMission(3模式×3档位，6步金标准动态教程) + Y9 id=9148（不放回抽签，Medium）+ id=9149（颜色配对，Hard）| **218关, 2314 tests ✓** |
| 2026-03-28 | **v9.10.0** | **SEQUENCE_NTH 新题型（等差+等比第n项）**: 基础设施已由并行session完成；本轮新增 Y10 id=1055（兵力预测，等差 Medium）+ id=1056（瘟疫蔓延，等比 Hard）+ Y11 id=1133（军饷翻倍，等比 Hard），动态6步教程；全年级教程审计 223关 100% 金标准 | **226关, 2339 tests ✓** |
| 2026-03-28 | **v10.1.0** | **COORD_3D 新题型 + 蜀道行军远征**: 三维坐标中点(midpoint+distance) 7文件全流程 + Y10 id=1066（城楼会师）+ id=1067（烽火定位）+ 蜀道远征8节点（荆州→成都，Y8-10，李白引语）| **228关, 2364 tests ✓** |
| 2026-03-28 | **v10.2.0** | **VECTOR_3D 新题型（三维向量加法）**: 7文件全流程 + generateVector3DMission(3档位含负分量，6步教程) + Y12 id=1241（空间行军 Medium）+ id=1242（炮火交叉 Hard含负）| **230关, 2389 tests ✓** |
| 2026-03-28 | **v10.3.0** | **Y12 金标准教程 + 积分面积**: VECTOR_3D id=1241/1242 补齐6步金标准教程（司马昭/姜维叙事）+ INTEGRATION id=1251（屯田面积 ∫x dx=18，三角形验算）+ id=1252（城墙曲面 ∫3x²dx=27，导数反验���）| **232关, 2389 tests ✓** |
| 2026-03-28 | **v10.4.0** | **严苛审查 + Y11/Y12加厚 + 生成器多样化 + Season 2**: data对齐修复(Y11 1182 INTEGRATION bounds + Y9 9123 target字段) + 10个Y9叙述者统一 + 翻译补全(SEQUENCE_FORMULA/PROBABILITY_TREE) + Y11 5新关(Unit 1/2/8/10/11各+1) + Y12 3新关(Unit 1/2/5各+1) + DERIVATIVE 1→3函数(x²/x³/2x²+1) + INTEGRATION 3→4函数(+4x³) + Season 2完整数据(12任务+30级奖励) + seasonTracker多赛季重构 + DailyQuest/BattlePass解耦 | **241关, 2389 tests ✓** |
| 2026-03-28 | **v10.4.1** | **Pre-testing polish**: ARIA无障碍(5文件 dialog/alert/input) + CIE课纲标签(Practice/Battle显示topic) + 教师看板行动建议(5类alert含suggestion) + SECTOR数据修复(10121/10122 pi/answer) + ROOTS/VENN partial credit + 7个TS strict错误清零 + 触摸目标≥44px + generator try-catch + tsconfig exclude dist/ + version v10.4 + Discover phase集成 + 186条错误反馈灵魂改写 | **241关, 2389 tests ✓** |
| 2026-03-28 | **v10.5.0** | **教师看板 Phase D1**: 班级邀请码系统（teacher_classes 表 + join_class_by_code RPC + ClassManager + JoinClassModal）+ 学生端"我的作业"面板（MyAssignments + get_my_assignments RPC）+ 教师白名单更新 | **241关, 2389 tests ✓** |
| 2026-03-29 | **v10.5.0** | **IGCSE KP全覆盖 + KP图谱 + 词汇系统**: 128新关卡(300→428), KP覆盖287/288=99.7%, KP知识图谱362边(前置/后续/根因), 917词身份档案+三层渐进提示+间隔重复, SINE_COSINE_RANDOM, 51MC生成器 | **428关, 2414 tests ✓** |
| 2026-03-30 | **v10.5.1** | **移动端 UX 打磨**: 顶部控件(语言切换+退出)3秒无操作自动收起为小圆点(opacity+scale 300ms过渡, hover/click展开) + 7个组件关闭/返回按钮扩大至44×44px最小触摸目标(VocabPopup/VocabReviewPanel/LearningTimeline/AssignmentPanel/RecoveryPathPanel) + BugReportButton手机端上移避开BottomNav + AchievementCard小屏安全边距 | **424关, 2414 tests ✓** |
| 2026-03-30 | **v10.5.2** | **性能收口（主包 + 教师路径）**: `skillHealth.ts` 轻量拆分，`processAttempt` 改为按需加载，`buildRecoveryPath()` 改为异步按需加载重依赖；App 主入口移除 resilience/error 链同步依赖，教师看板切到 `useMissions(grade)` 年级懒加载；仅剩 `LearningTimeline` 保留全量 missions 静态导入 | **424关, 2414 tests ✓** |
| 2026-03-30 | **v10.5.3** | **性能收口（最后一个全量 missions 依赖）**: `LearningTimeline` 移除对 `../data/missions` 的静态导入，改为打开时间线时按需加载各年级 mission title map 并缓存；运行时 `screens/components` 已无全量 `MISSIONS` 静态依赖 | **424关, 2414 tests ✓** |
| 2026-03-30 | **v10.5.4** | **MissionSummary 数据层收口**: 新增 `scripts/generate-mission-summaries.ts` + `src/data/missionSummaries/*` 生成产物，教师看板切到 `useMissionSummaries(grade)`；`LearningTimeline` / `MyAssignments` / `AssignmentBanner` 统一消费 summary 数据；新增 `missionSummary.test.ts` 锁定 generated summaries 与 mission 源数据一致 | **424关, 2420 tests ✓** |
| 2026-03-30 | **v10.5.5** | **MapScreen 弹层懒加载**: `SkillTreePanel / EquipmentPanel / InventoryPanel / ShopPanel / BattlePassPanel / StrategicScrollsPanel / ProgressReport / MyAssignments` 切到 `React.lazy + Suspense`；地图首屏只保留主流程，非首屏弹层打开时再下载 | **MapScreen 114.45→68.20 kB, 2420 tests ✓** |
| 2026-03-30 | **v10.5.6** | **恢复力补强 + 作业缺失兜底**: summary loader 失败后不再缓存 reject，`useMissionSummaries()` 增加错误态；Map lazy 面板统一 loading overlay；作业入口改为 `missionId -> loadMissionById()`；`AssignmentBanner/MyAssignments` 对缺失 summary 先显示占位标题再补加载 missing summaries；新增 2 条回归测试 | **MapScreen 68.20→69.50 kB, 2422 tests ✓** |
| 2026-04-03 | **v10.9.1** | **学生账号批量创建工具**: SQL + TS 双脚本（`scripts/create-students-2026-04-03.sql` / `scripts/create-students.ts`），支持一键创建 auth 用户 + profile + 班级邀请码；本批：julia.feng(Y9/9A) + ruining.jia(Y9/9B) + bonnie.ma(Y8/8A) | **424关, 2422 tests ✓** |
| 2026-04-07 | **v10.10.0** | **meta_node_progress 统一进度表**: Supabase 迁移(CREATE TABLE + upsert RPC + get_student RPC + RLS) + App.tsx 成功/失败路径 fire-and-forget 写入 + StudentDetailCard 统一掌握度面板(Play/Practice/Mastered) + KPWeaknessPanel 跨产品掌握度(kn_id→mastery_score) + MetaNodeProgressRow 类型 | **424关, 2687 tests ✓** |
| 2026-04-07 | **v10.11.0** | **Mock 模拟卷数据接入**: KPWeaknessPanel source-prioritized badge(mock>practice>play, 颜色编码) + StudentDetailCard 4列统一掌握度(Mock/Practice/Play/Mastered) + Mock历史面板(按日期分组+kn_id得分+进步趋势) | **424关, 2687 tests ✓** |
| — | **v10.6.0** | **教师看板 Phase D1 续**: 作业分享链接 + 地图作业角标 + 截止倒计时（详见 docs/TEACHER-DASHBOARD-PLAN.md）|
| — | **v10.7.0** | **教师看板 Phase D2**: 今日摘要卡片 + 按 KP 筛选弱生 + 一键补习作业 |
| — | **v10.8.0** | **教师看板 Phase D3**: 班级周报 + 家长月报 + 8 周趋势图 |
| — | **v11.0.0** | **用户验证 + 学校试点**: 详见 docs/HONEST-ASSESSMENT.md + docs/USER-TEST-PLAYBOOK.md |

---

## 赛季 2 内容规划（Season 2 Road Map）

> **版本目标**: v9.8.0–v10.0.0 | **预计发布**: 2026-04-15
> **赛季主题**: 赤壁之战后·西征蜀道（从赤壁胜利到成都入城）
...
| 年级 | 数量 | 主题 | 关键知识点 |
|------|------|------|------------|
| Y10 | 5 | 相似三角形 + 三维坐标 + 放大变换 | SIMILAR_TRIANGLES / 3D_COORD / ENLARGEMENT_RANDOM |
| Y11 | 5 | 数列 + 复数 + 参数方程 | SEQUENCE_RANDOM / 高级函数 |
| Y12 | 5 | 积分面积 + 向量3D + 二阶导数 | INTEGRAL_AREA / VECTOR_3D / SECOND_DERIV |
| Y9  | 5 | 频率直方图 + 树形图概率 | HIST_FREQ / TREE_DIAGRAM |

#### 三币商店（v9.4.0）

| 商品 | 价格（功勋） | 效果 |
|------|------------|------|
| 🛡️ 护盾卷轴 | 80 功勋 | 下场闯关免一次错误 |
| ⚡ 双倍令牌 | 100 功勋 | 下场闯关得分×2 |
| 🔮 透视符 | 60 功勋 | 下场闯关提前看答案范围 |
| 🔧 修复道具 | 120 智略 | 立即修复一件受损装备 |
| 📜 知识祝福 | 200 功勋 | 随机一件装备耐久+7天 |

**军粮**（第三种货币）：从每日试炼发放（每日+3），专用于远征模式消耗。

#### 赛季 2 任务体系（10个里程碑）

| # | 任务 | 奖励 |
|---|------|------|
| 1 | 连续7天登录 | 称号"坚忍之将" + 500功勋 |
| 2 | 累计闯关100场 | 称号"百战将军" + 1000功勋 |
| 3 | 练习总题数突破500 | 称号"勤修苦练" + 200智略 |
| 4 | 首次三币商店消费 | 称号"军需官" + 解锁商店折扣 |
| 5 | Y7-Y9 全关卡通关 | 称号"中原已定" + 武将技能点+1 |
| 6 | Y10-Y12 至少通关10关 | 称号"问鼎中原" + 特殊边框 |
| 7 | 赤壁远征通关 | 称号"赤壁英雄" + 500功勋 |
| 8 | PK赢得10场 | 称号"沙场宿将" + 特殊头像框 |
| 9 | 连胜宝箱8连胜 | 称号"连战连捷" + MEGA宝箱 |
| 10 | 完成所有赛季任务 | 称号"三国统帅" + 赛季2专属皮肤 |

#### 新远征：蜀道行军（赤壁第二章）

- **8个节点**: 荆州→汉中→剑门关→成都
- **特色机制**:
  - "雾霾关卡"：生成器tier=hard，题目无法提前预知
  - "魏军突袭"：随机插入一道额外题（如答对+2军粮）
  - **Boss节点**: 曹仁守城（5题连答，2错即失败）

#### 新生成器（优先级排序）

| 优先级 | 类型 | 覆盖 |
|--------|------|------|
| HIGH | `SIMILAR_TRIANGLES` | Y9/Y10 相似三角形 |
| HIGH | `TREE_DIAGRAM` | Y9/Y10 树形图概率 |
| MEDIUM | `SEQUENCE_NTH` | Y10/Y11 等差等比数列第n项 |
| MEDIUM | `VECTOR_3D` | Y12 三维向量 |
| LOW | `INTEGRAL_AREA` | Y12 积分求面积 |

### 赛季 2 技术架构升级

#### 1. 三币商店（v9.4.0）
```
src/components/CurrencyShopPanel.tsx  ← 新建
src/utils/currency.ts                 ← 扩展 spendCurrency()
src/data/shopItems.ts                 ← 商品注册表
```

#### 2. 新远征（v9.8.0）
```
src/data/expeditions/shuDao.ts   ← 蜀道远征 8 节点
src/utils/expeditionEngine.ts    ← 提取为独立工具（现在混在 ExpeditionScreen）
```

#### 3. 网络对战优化（v10.0.0前）
```
Supabase Realtime: gl_pk_sessions 改用 LISTEN/NOTIFY
目标延迟: <200ms (当前架构为轮询)
```

### 赛季 2 质量门禁（发布前 checklist）

```bash
# 1. 新关卡全部达金标准（≥6步+WHY+验算）
# 2. 三币经济平衡测试（新号30分钟内是否能获得第一件商品）
# 3. 远征模式通关率测试（目标: 熟练生 60-70% 通过率）
# 4. 新生成器无除零守卫问题
# 5. Vitest 测试数 ≥ 2300
# 6. build 零错误 + lint 零 warning
```
