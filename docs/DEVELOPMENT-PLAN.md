...
| 2026-03-27 | **v9.1.1** | **状态层安全修复（2项全局 CRITICAL）**: 9处 `structuredClone` 深拷贝替换浅拷贝（防 `_equipment`/`_mistakes` 嵌套 mutation 竞态，规则 J）+ 7处 `latestScoreRef.current` 替换 `profile.total_score`（防 async 闭包 XP 丢失，规则 K）| **Build 零错误，2164 tests 全通过** |
