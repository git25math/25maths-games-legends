...
| # | 任务 | 详情 | 验收标准 | 工程量 |
|---|------|------|---------|--------|
| 2.1 | **生成器参数化** | mission story/description 用 `{a}`, `{result}` 占位符，生成器只更新 data 中的数字字段，render 时 `interpolate(story, data)` 插值。title 永远不变。已完成 SIMPLE_EQ 4 个 mission 的 POC，剩余 20 个待转换。 | 同一 mission 多次进入：标题不变、故事角色不变、只有数字不同 | 中（POC 已验证，剩余是重复劳动） |
| 2.2 | **答错解题展示** | 答错后展开解题面板：① 你的答案 vs 正确答案 ② 逐步解题过程（复用 tutorialSteps）③ "再试一次"按钮。练习模式中答错不扣血，闯关模式中扣血但仍展示解题。 | 答错后能看到"你答了 8，正确答案是 7，因为 12-5=7" | 中 |
| 2.3 | **练习模式** | 新增 PracticeMode 组件：① 选一个 KP ② 从 Green 开始，无限做同类型题（每题新数字）③ 学生自主决定何时升级（[我准备好了→Amber]）④ 可随时退回上一阶段 ⑤ Red 通过后该 KP 标记为"可闯关"。无 HP、无计时、答错展示解题过程后继续。 | 学生自主完成 Green→Amber→Red 递进，点"进入闯关"解锁 battle | 大 |
| 2.4 | **闯关模式改造** | 现有 MathBattle 改为"闯关模式"：前置条件检查（该 KP 练习通关）+ 多题连闯（5 题，同类型不同数字）+ HP + 计时 + 结算。gl_battle_results 加 session_id。 | 练习未通关的 KP 闯关按钮显示🔒 | 大 |
| 2.5 | **连击 + 伤害数字飞出** | 连对 3 题 → ×1.5, 5 题 → ×2。答对瞬间金色 "+250" 上飘淡出。仅闯关模式生效。 | 300ms 内视觉反馈完成 | 小 |
| 2.6 | **移动端响应式** | 练习模式 + 闯关模式 < 768px 单列，输入框全宽，按钮 min-h-12 | iPhone SE 375px 可用 | 中 |
| 2.7 | **内部验证** | 3 个学生试玩练习→闯关完整流程 15 分钟 | 观察记录：是否理解"先练后闯"、哪里卡住 | — |

**实施顺序**：
`2.1(参数化剩余 20 missions) → 2.2(答错解题) → 2.3(练习模式) → 2.4(闯关模式改造) → 2.5(连击+特效) → 2.6(移动端) → 2.7(验证)`

> 全部为纯前端工作。唯一 DB 改动（session_id）在 2.4 时做。
